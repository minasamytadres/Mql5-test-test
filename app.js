// ============================================
// MQL5 Backtest Engine - v3 Fixed Transpiler
// ============================================
...
function getSampleGoldData() {
    log('⚠️ استخدام بيانات نموذجية للذهب');
    const data = [];
...
function getSampleEURUSDData() {
    log('⚠️ استخدام بيانات نموذجية لـ EURUSD');
    const data = [];
...
async function fetchYahooData(symbol, interval, range) {
    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 404) throw new Error('الزوج غير موجود');
        ...
        log('✅ تم تحميل ' + bars.length + ' شمعة من Yahoo');
        return bars;

    } catch (err) {
        log('⚠️ فشل الاتصال: ' + err.message);
        log('🔄 استخدام البيانات الاحتياطية...');
...
function transpileMQL5(code) {
    ...
    // Replace input declarations: input int FastMA = 10; → var FastMA = 10;
    js = js.replace(/\binput\s+(?:int|double|string|bool|datetime|color)\s+/g, 'var ');

    // Replace function declarations
    js = js.replace(/\bvoid\s+OnTick\s*\(\s*\)/g, 'function onTick()');
    js = js.replace(/\bvoid\s+OnInit\s*\(\s*\)/g, 'function onInit()');
    js = js.replace(/\bvoid\s+OnDeinit\s*\(\s*\)/g, 'function onDeinit()');
    js = js.replace(/\bint\s+OnInit\s*\(\s*\)/g, 'function onInit()');
    js = js.replace(/\bvoid\s+OnTimer\s*\(\s*\)/g, 'function onTimer()');

    // Replace MQL5 type keywords before variable names (inside functions)
    // double fastMA = ... → var fastMA = ...
    // int ticket = ... → var ticket = ...
    js = js.replace(/\b(int|double|string|bool|datetime|color)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=/g, 'var $2 =');

    // Replace MQL5 functions with engine calls
    const reps = [
...
`;
        ...
        const func = new Function('engine', fullScript + 'return {onInit: typeof onInit === "function" ? onInit : null, onTick: typeof onTick === "function" ? onTick : null, onDeinit: typeof onDeinit === "function" ? onDeinit : null};');
        const handlers = func(engine);

        if (handlers.onInit) {
            log('🔧 تشغيل OnInit...');
            handlers.onInit.call(engine);
        }
        ...
        for (let i = 0; i < totalTicks; i++) {
            ...
            if (handlers.onTick) handlers.onTick.call(engine);
        ...
        if (handlers.onDeinit) handlers.onDeinit.call(engine);
